...
If you [follow me on Twitter](https://twitter.com/fortysevenfx), you may be aware of my weird [weekend projects](https://twitter.com/search?q=%40fortysevenfx%20%22weekend%20project%22).
...
There are many solutions out there: WordPress' hashicons, roboicons, monsters etc..

I wanted one that still looks abstract (not as opinionated as monsters or robots),
but that plays nice in a rounded avatar UI component.
...
If you're interested to see how it's done, read on. Otherwise you can just
play with a more detailed [interactive example here](/hashvatar).

## Space Partitioning

<Note title="Content warning" icon={FiAlertTriangle}>
  Maths &amp; trigonometry.
</Note>

Cartesian (X-Y) approaches would lose information on the corners when rounded,
so we're going to use polar (angle-radius) coordinates instead.
...
Doing this naively, with each concentric circle's radius being 1/4th of the
...
There are some issues: the innermost circle sections are tiny compared to the outermost.

If we try to calculate the radii so that each section has an equal area, we get
...
We can split it in 32 blocks of 8 bits (2 hexadecimal digits), and map them
to the circle sections:

```
31 5f 5b db 76 d0 78 c4  Outer circle
3b 8a c0 06 4e 4a 01 64  Middle-outer circle
61 2b 1f ce 77 c8 69 34  Middle-inner circle
5b fc 94 c7 58 94 ed d3  Inner circle
...
In order to fill each section with a different color, we generate an SVG `<path>` polygon.
...
For that, we have many options. 8 bits could map directly to 256 colors like the
old Windows systems, but that would require a lookup table. Instead, we can
generate colours using the HSL CSS function.
...
function mapValueToColor(value) {
  ...
  const l = 40 + 30 * normalizedL // Lightness between 50 and 80%
  return `hsl(${h}, ${s}%, ${l}%)`
}
```
...
<Note title="About accessibility" status="info">
  The color mapping function could use a high-contrast version that focuses on
...
if difference occur in the LSBs of hue/saturation/lightness components.
...
function useSouls(bytes: string[]) {
...
These values give us additional parameters to play with in the color calculation.
  ...
  <Flex justifyContent="space-around" mb={2}>
    <SHA256Avatar
      ...
      mapColor={function mapValueToColor({ value }) {
        ...
        const h = 360 * normalizedH
        const s = 50 + 50 * normalizedS // Saturation between 50 and 100%
        const l = 50 + 20 * normalizedL // Lightness between 50 and 70%
        return `hsl(${h}, ${s}%, ${l}%)`
      }}
    />
    <SHA256Avatar
...
with accessibility for color-blind people
...
With a bit of tweaking in the color mapping value, we can easily extend this
...
[code for this article](https://github.com/franky47/francoisbest.com/blob/next/src/pages/posts/2021/hashvatars.mdx) is on GitHub, as well as the [component itself](https://github.com/franky47/francoisbest.com/blob/next/src/components/Sha256Avatar.tsx).

[Follow me on Twitter](https://twitter.com/fortysevenfx) for updates.
