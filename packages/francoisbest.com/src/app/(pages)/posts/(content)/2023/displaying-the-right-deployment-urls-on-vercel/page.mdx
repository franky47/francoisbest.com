...
import { url, resolve, filePathToUrlPath } from 'lib/paths'

## The issue

<Note status="info" title="Are we there yet?">

Is this the current URL to this post?

<pre>{url(filePathToUrlPath(resolve(import.meta.url)))}</pre>

If yes, then everything works fine. If no, read on.
...
Setting the [`VERCEL_FORCE_NO_BUILD_CACHE`](https://vercel.com/docs/deployments/troubleshoot-a-build#managing-build-cache)
environment variable to `1` for deployments does not seem to have an effect.
